...
class OllamaModelLoader:
    async def ensure_model_available(self, model_name: str):
        # Example: check if model is available via Ollama API
        # You can use subprocess or requests to query `ollama list`
        print(f"Ensuring model '{model_name}' is available...")
        # Raise exception or pull model if needed
class SkynetLite:
    ...
    async def initialize(self) -> bool:
        ...
        if not await self.model_loader.ensure_model_available("mistral")
            return False
