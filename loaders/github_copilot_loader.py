...
class GitHubCopilotModelLoader:
    """Minimal GitHub Copilot-compatible client using a configurable endpoint.
    ...
    async def initialize(self) -> bool:
        if not self.token or not self.api_url:
            logger.info("GitHub Copilot token or API URL not configured; loader disabled")
            return False
        self.client = httpx.AsyncClient(timeout=30.0)
        logger.info("GitHub Copilot loader initialized")
        return True
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    backoff *= 2
                    continue
                logger.error("GitHub Copilot API failed after retries")
                return ""
            except Exception as e:
                logger.error(f"Unexpected error calling GitHub Copilot endpoint: {e}")
                return ""
