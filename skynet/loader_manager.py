...
class LoaderManager:
	...
	async def initialize(self):
		...
		if os.getenv('GITHUB_COPILOT_TOKEN'):
			try:
				from loaders.github_copilot_loader import GitHubCopilotModelLoader
				self.loader = GitHubCopilotModelLoader()
				if not await self.loader.initialize():
					raise RuntimeError("Failed to initialize GitHub Copilot loader")
				logger.info("Using GitHubCopilotModelLoader")
				return True
			except (ModuleNotFoundError, ImportError) as e:
				logger.warning(f"GitHubCopilotModelLoader not available: {e}")
			except Exception as e:
				logger.error(f"Failed to initialize GitHubCopilotModelLoader: {e}")
